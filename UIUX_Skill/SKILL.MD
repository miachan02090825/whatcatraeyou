---
name:ui-eval
description:本技能提供建立 Web 與移動端應用介面的綜合指南，包含樣式選擇、UX 最佳實踐與實作檢查清單。
---
...
配色 (Colors): 定義主色 (Primary)、強調色 (Accent) 與背景色，確保深淺模式對比度充足。
排版 (Typography): 選擇標題與內文的字體搭配。
陰影與邊框 (Effects): 統一使用圓角 (Radius) 與陰影 (Shadow) 風格。
避免模式 (Anti-patterns): 識別應避免的設計錯誤。
...
視覺品質: 顏色一致？間距是否遵循柵格系統？
互動: Hover 狀態是否明顯？按鈕是否有 Active 效果？
深色模式: 切換到深色模式檢查是否有文字看不見或對比度不足。
無障礙 (a11y): 圖片是否有 Alt？按鈕是否有 Aria-label？
響應式: 在手機尺寸下是否破版？